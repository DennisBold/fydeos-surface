...
mtk_cam_cio_get_active_sensor(struct mtk_cam_dev *cam)
{
	...
	media_device_for_each_entity(entity, mdev) {
		dev_dbg(dev, "media entity: %s:0x%x:%d\n",
			...
			media_entity_is_streaming(entity));
				       ...
				       unsigned int count)
{
	...
	if (!media_entity_is_streaming(&node->vdev.entity)) {
		ret = media_pipeline_start(node->vdev.entity.pads, &cam->pipeline);
		if (ret) {
...
static int mtk_cam_v4l2_async_register(struct mtk_cam_dev *cam)
{
	...
	ret = v4l2_async_nf_parse_fwnode_endpoints(dev,
		&cam->notifier, sizeof(struct v4l2_async_connection), NULL);
	if (ret) {
